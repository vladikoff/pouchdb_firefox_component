...
const {classes: Cc, interfaces: Ci, results: Cr, utils: Cu,components: components } = Components;
var trackedObjects = {};
...
memory.track = function track(object, bin, stackFrameNumber) {
...
memory.getBins =  function getBins() {
  var names = [];
  for (let name in trackedObjects)
    names.push(name);
  return names;
...
memory.getObjects = function getObjects(bin) {
